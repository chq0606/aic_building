...
def list_profiles(tenant_id: str, template_type: str | None = None) -> list[dict]:
    """列出租户的所有 mapping_profile。"""
    with get_conn() as conn:
        with conn.cursor() as cur:
            if template_type:
                cur.execute("""
                    SELECT id, profile_name, template_type, created_at
                    FROM ingest.mapping_profile
                    WHERE tenant_id = %s AND template_type = %s
...
def get_profile(profile_id: str, tenant_id: str) -> dict | None:
    """取单个 profile。tenant_id 校验避免跨租户访问。"""
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT id, profile_name, template_type, timezone, timestamp_format,
                       mapping_json, unit_rules, created_at
                FROM ingest.mapping_profile

...
| 园区探索页 | 分析中心 |
|:---:|:---:|
| ![园区探索页](图片/运行图片/园区探索页.png) | ![分析中心](图片/运行图片/分析中心.png) |
| **楼层分析** | **数据接入** |
| ![楼层分析](图片/运行图片/楼层分析.png) | ![数据接入](图片/运行图片/数据接入.png) |
...
| **能耗分析** | 能耗排名 / 多楼时序对比 / EUI 基准（对标 GB 55015 强制限值）/ 异常事件汇总 / 能耗-天气关联 / 数据质量面板 / 预测对比，共 7 类图表 |
...
> 更完整的启动与使用说明（本地开发模式、GPU 环境自查、常见报错排查、demo 账号权限边界）见 **[使用说明.md](使用说明.md)**。
...
本项目源于第八届全球校园人工智能算法精英大赛的参赛作品，围绕建筑能耗的采集、可视化、预测与 AI 节能优化展开。
...
本项目在开发过程中使用 AI 编程助手（Claude Code 框架 + GLM-5.2 大模型）辅助实现代码，技术选型、架构设计、需求评审、代码审查与验收均由人工完成。开发过程中使用的提示词见 [构建思路/提示词.txt](构建思路/提示词.txt) 与 [构建思路/提示词1.txt](构建思路/提示词1.txt)。
